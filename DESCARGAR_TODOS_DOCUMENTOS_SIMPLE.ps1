...
for ($i = 1; $i -le $totalBatches; $i++) {
    ...
    try {
        $result = Invoke-RestMethod -Uri "http://localhost:5265/api/documents/download-batch?batchNumber=$i&batchSize=$batchSize&delayMs=$delayMs" -Method POST -ErrorAction Stop
        ...
        # Estadisticas acumuladas cada 10 batches
        if ($i % 10 -eq 0) {
